...
This module provides functions for visualising and calculating statistics for RIE singular value cleaning of lagged covariance matrices.
It includes functionalities to plot various types of graphs, calculate summary statistics, and reconstruct matrices.
"""
...
# Configuration settings for figure dimensions, display options, and testing
figsize = (12, 8)                    # Default figure size for plots
do_save = 1                          # Flag to control saving of figures (1 = save, 0 = do not save)
do_show = 0                          # Flag to control display of figures (1 = show, 0 = do not show)
do_test = 0                          # Flag to enable testing mode for reduced data set sizes

# Models configuration
models = ['Random Gaussian', 'AR']   # List of statistical models to be visualised
...
def Total_Cov(model, n, p, T):
    ...
    Parameters:
    - model (str): Model type ('Random Gaussian', or 'AR').
    - n (int): Number of variables in the model.
    - p (int): Parameter specific to the model (unused in some models).
    - T (int): Number of time steps.

    Returns:
    - tuple: A tuple containing the model's theoretical true singular values and covariance matrices.
...
def reconstruct_matrix(U, Clean_s, V):
    # Construct a diagonal matrix from the cleaned singular values
    Diag_Clean_s = np.diag(Clean_s)

    assert U.shape[1] == Diag_Clean_s.shape[0] == Diag_Clean_s.shape[1] == V.shape[0], "Matrix dimensions not compatible for reconstruction"
    
    # Note that V is already in its transposed form
...
def show_save_close(my_model, title, dimension_details):
    # Display, save, show, and close the plot
    if do_save:
...
def plot_matrix_heatmaps(my_model, true_matrix, empirical_matrix, cleaned_matrix, dimension_details,
    ...
    # Create the figure and gridspec for multiple subplots
...
def calculate_signal_to_noise(matrix):
    ...
    var_off_diag = np.var(off_diag_elemetns, ddof=1) / len(off_diag_elemetns)
...
for my_model in models:
    ...
    for n, p, T in zip(ns, ps, Ts):
        ...
        dimension_details = f"T={str(T)}_n={str(n)}"

        # Calculate the empirical lagged covariance matrix, the true singular values, and the cleaned singular values
        ...
        # Get the submatrices of the lagged covariance matrix, the true matrix, and reconstruct the cleaned matrix
        ...
        assert np.allclose(cleaned_matrix, reconstructed_CXY), "There are issues with reconstruction"

        # Calculate the frobenius norms, diagonal differences, and Signal to Noise Ratios
        ...
        diag_abs_diff_true_vs_empirical = np.abs(diag_elements_true - diag_elements_empirical)
        diag_abs_diff_true_vs_cleaned = np.abs(diag_elements_true - diag_elements_cleaned)
        diag_abs_diff_true_vs_isotonic_cleaned = np.abs(diag_elements_true - diag_elements_isotonic_cleaned)
        ...
        diag_diff_true_vs_empirical.append(np.sum(diag_abs_diff_true_vs_empirical))
        diag_diff_true_vs_cleaned.append(np.sum(diag_abs_diff_true_vs_cleaned))
        diag_diff_true_vs_isotonic_cleaned.append(np.sum(diag_abs_diff_true_vs_isotonic_cleaned))

        SNR_emps.append(SNR_emp)
        SNR_cleaneds.append(SNR_cleaned)
        SNR_isotonic_cleaneds.append(SNR_isotonic_cleaned)

        # Print the summary statistics for table comparisons
        ...
        print(f"Mean absolute difference between the true and empirical diagonal elements: {np.mean(diag_abs_diff_true_vs_empirical):.5f}")
        print(f"Mean absolute difference between the true and cleaned diagonal elements: {np.mean(diag_abs_diff_true_vs_cleaned):.5f}")
        print(f"Mean absolute difference between the true and isotonic cleaned diagonal elements: {np.mean(diag_abs_diff_true_vs_isotonic_cleaned):.5f}")
        ...
        # Run the visualisation functions
        if my_model == 'AR':
    ...
    # Run the summary statistics visualisation functions
